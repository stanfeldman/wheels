...
({
	initialize: function(options)
	{
		...
		this.router = new controllers.Router(this.options.views.static_path);
		this.text_viewer = new views.TextViewer(this.options.views.template_path);
		this.translator = new views.Translator(this.options.views.locale_path);
...
({
	...
	emit: function(event)
	{
		...
		if(!found)
		{
			if(!["not_found", "before_action", "after_action", "before_model_save", "after_model_save", "before_model_remove", "after_model_remove"].contains(event))
			{
				console.log(event);
