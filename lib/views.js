...
({
	initialize: function()
	{
		...
		var template_path = core.Application.options.views.template_path;
		var finder = findit.find(path.normalize(template_path));
		finder.on('file', function (file) 
		{
			var filepath = path.normalize(file);
			console.log(filepath.substring(path.normalize(template_path).length));
			var data = fs.readFileSync(filepath, 'utf-8');
			var compiled = TemplateEngine.compile(data, filepath.substring(path.normalize(template_path).length));
	...
	render: function(req, res, context)
	{
		TemplateEngine.render(context.template_name, context, function(err, out) 
		{
			...
			res.end(out);
			//core.Eventer.instance().emit("after_action", req, res);
		});
	}
});
...
({
	initialize: function()
	{
		...
		this.translations = {};
		var files = fs.readdirSync(path.normalize(core.Application.options.views.locale_path));
		for(var i = 0; i < files.length; ++i)
		{
			var item = files[i];
			var data = fs.readFileSync(core.Application.options.views.locale_path+item, 'utf-8');
