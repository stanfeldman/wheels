...
({
	initialize: function(template_path)
	{
		if(typeof TextViewer.instance == "object")
			return TextViewer.instance;
		this.template_path = template_path;
		var finder = findit.find(path.normalize(this.template_path));
...
({
	initialize: function(locale_path)
	{
		if(typeof Translator.instance == "object")
			return Translator.instance;
		this.translations = {};
		this.locale_path = path.normalize(locale_path);
		var files = fs.readdirSync(this.locale_path);
		for(var i = 0; i < files.length; ++i)
		{
			var item = files[i];
			var data = fs.readFileSync(locale_path+item, 'utf-8');
