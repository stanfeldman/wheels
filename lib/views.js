...
({
	initialize: function()
	{
		...
		var template_path = new core.Application().options.views.template_path;
...
({
	initialize: function()
	{
		...
		this.translations = {};
		var files = fs.readdirSync(path.normalize(new core.Application().options.views.locale_path));
		for(var i = 0; i < files.length; ++i)
		{
			var item = files[i];
			var data = fs.readFileSync(new core.Application().options.views.locale_path+item, 'utf-8');
